...
export default function Editor() {
  ...
  function arrRemove(key, i) {
    const next = { ...contentRef.current, [key]: contentRef.current[key].filter((_, idx) => idx !== i) };
    applyContent(next);
    writeDraft(next);
  }
  ...
  if (error && !contentRef.current) return <p className="admin-loading">{error}</p>;
    ...
    <div className="admin-split">
      ...
      <aside className="admin-split__sidebar">
        ...
        <div className="admin-save-bar">
          ...
          <button type="button" className="admin-btn admin-btn--ghost admin-btn--sm" onClick={handleReset} title="Reload saved content and reset preview">
            Reset to Saved
          </button>
      ...
      <div className="admin-split__preview">
        <div className="admin-split__preview-bar">
          ...
          <span className="admin-split__preview-hint">Changes appear instantly as you edit</span>
        </div>
        <iframe
          ref={iframeRef}
          src="/"
